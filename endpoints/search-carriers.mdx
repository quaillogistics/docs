...
<Note>
  **Every result carries its risk score.** Each row includes `risk_score`, `risk_grade`, and `risk_tier`, so you can rank or filter a page of results without a follow-up [Get Carrier](/endpoints/get-carrier) call per row.

  All three are `null` when the carrier has no computed score. `risk_score` of `0` is a real score — the worst one — so test for `null` explicitly rather than for falsiness.

  The full component breakdown and risk flags stay on the carrier profile. Use search to narrow, then fetch the profile for the few carriers you care about.
</Note>
